...
import { publicKey } from "@oysterr/common/dist/lib/utils/layout";
...
export const mintNFT = async (
  ...
  console.log('mintNFT.1', publicKey);
  ...
  console.log('mintNFT.2', metadataContent);

  const realFiles: File[] = [
    ...files,
    new File([JSON.stringify(metadataContent)], RESERVED_METADATA),
  ];
  console.log('mintNFT.3', realFiles);

  const { instructions: pushInstructions, signers: pushSigners } = await prepPayForFilesTxn(wallet, realFiles);
  console.log('mintNFT.4');

  progressCallback(1);
  console.log('progressCallback 1', '');

  const TOKEN_PROGRAM_ID = programIds().token;
  console.log('mintNFT.10.TOKEN_PROGRAM_ID', TOKEN_PROGRAM_ID);
  ...
  console.log('recipientKey.6', recipientKey);
  ...
  const metaData: DataV2 = new DataV2({
    symbol: metadata.symbol,
    name: metadata.name,
  ...
  progressCallback(2);
  console.log('progressCallback 2', metadataAccount);
  ...
  progressCallback(3);
  console.log('progressCallback 3', txid);

  try {
    await connection.confirmTransaction(txid, 'max');
    progressCallback(4);
    console.log('progressCallback 4', 'confirmTransaction');
  } catch {
    // ignore
  }
  ...
  progressCallback(5);
  console.log('progressCallback 5', 'getParsedConfirmedTransaction');
  ...
  console.log('uploadToArweave.data', data);

  // TODO: convert to absolute file name for image
  const result: IArweaveResult = await uploadToArweave(data);
  progressCallback(6);
  console.log('progressCallback 6', result);

  const metadataFile = result.messages?.find(
    m => m.filename === RESERVED_TXN_MANIFEST,
  );

  let arweaveLink: string = '';
  if (metadataFile?.transactionId && wallet?.publicKey) {
    ...
    console.log('progressCallback.arweaveLink', arweaveLink);
    ...
    progressCallback(7);
    console.log('progressCallback 7', updateInstructions);
    ...
    */
    ...
    progressCallback(8);
    console.log('progressCallback 8', 'createMasterEditionV3');
  ...
  console.log('progressCallback success', metadataAccount);
  return { metadataAccount, txid, arweaveLink, recipientKey, mintKey };
};
...
export const prepPayForFilesTxn = async (
  ...
  const lamport: string = await (await getAssetCostToStore(files)).toString();
  console.log('prepPayForFilesTxn.1', wallet.publicKey, AR_SOL_HOLDER_ID, parseInt(lamport));
  if (wallet.publicKey)
    instructions.push(
      SystemProgram.transfer({
        fromPubkey: wallet.publicKey,
        toPubkey: AR_SOL_HOLDER_ID,
        lamports: parseInt(lamport),
      }),
    );

  console.log('prepPayForFilesTxn.2');
