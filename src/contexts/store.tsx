...
import { getStoreID, setProgramIds, StringPublicKey } from '@oysterr/common';
...
export const StoreProvider: FC<{
  ...
  const ownerAddressFromQuery = null;
  const initOwnerAddress = ownerAddressFromQuery || ownerAddress;
  console.log('StoreProvider.initOwnerAddress', initOwnerAddress);

  const initStoreAddress = !ownerAddressFromQuery ? storeAddress : undefined;
  console.log('StoreProvider.initStoreAddress', initStoreAddress);
  ...
  const setStoreForOwner = useMemo(
    () => async (ownerAddress?: string) => {
      console.log('setStoreForOwner.ownerAddress', ownerAddress);

      const storeAddress = await getStoreID(ownerAddress);
      console.log('setStoreForOwner.storeAddress', storeAddress);
